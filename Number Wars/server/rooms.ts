...
// Is this name already used by a DIFFERENT player in the room? (case-insensitive)
function nameTaken(room: ServerRoom, name: string, exceptId: string): boolean {
  const n = (name ?? '').trim().toLowerCase();
  if (!n) return false;
  return room.players.some((p) => p.id !== exceptId && (p.name ?? '').trim().toLowerCase() === n);
}
...
function commitRoom(io: Server, code: string, next: ServerRoom): void {
  ...
  // Frozen players who thawed during this move had their turn silently skipped —
  // tell everyone so the freeze feels real. (Game resets don't go through commitRoom.)
  const skipped = room.frozenPlayerIds.filter((id) => !next.frozenPlayerIds.includes(id));
  let result = next;
  ...
  for (const id of skipped) {
    const name = room.players.find((p) => p.id === id)?.name ?? 'A player';
    io.to(code).emit('card-effect', { kind: 'skip', byName: '', targetName: name });
  }
}
...
export function registerHandlers(io: Server): void {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join-room', ({ code, player }: { code: string; player: PlayerInput }) => {
      ...
      // No two players in a room may share a name (case-insensitive).
      if (nameTaken(room, player.name, player.id)) {
        socket.emit('name-taken', player.name);
        return;
      }
      if (existing) {
    ...
    socket.on('start-game', ({ code }: { code: string }) => {
      ...
      // Random team-up (teams mode): players don't pick — sides are drawn at start.
      // Requires an even split across all teams (e.g. 2-2, 3-3-3); otherwise can't start.
      if (room.settings.mode === 'teams' && room.settings.randomTeams) {
        const players = room.players.filter((p) => !p.isSeat);
        const tc = room.settings.teamCount;
        if (players.length < tc || players.length % tc !== 0) return; // must split evenly
        const colors = COLORS.slice(0, tc);
        const perTeam = players.length / tc;
        shuffle(players.map((p) => p.id)).forEach((pid, i) => {
          const p = room.players.find((pl) => pl.id === pid);
          if (p) p.team = colors[Math.floor(i / perTeam)];
        });
        Object.assign(room, startGame(room));
        armTurnTimer(io, code);
        broadcast(io, room);
    ...
    socket.on(
      ...
      ({ code, cardId, cellIndex }: { code: string; cardId: string; cellIndex: number }) => {
        ...
        const next = playPlusCard(room, actor, cardId, cellIndex);
        const ok = had && !(next.hands[actor] ?? []).some((c) => c.id === cardId);
        applyAndBroadcast(code, next);
        if (ok) io.to(code).emit('card-effect', { kind: 'plus', byName });
      }
    );
    ...
    const playSpecial = (
      ...
      const targetName = targetPlayerId
        ? room.players.find((p) => p.id === targetPlayerId)?.name
        : undefined;
      ...
      applyAndBroadcast(code, next);
      if (ok)
        io.to(code).emit('card-effect', { kind: shieldBroke ? 'shieldblock' : kind, byName, targetName });
    };
    ...
    socket.on(
      ...
      ({ code, cardId, targetPlayerId, cardIndex }: { code: string; cardId: string; targetPlayerId: string; cardIndex: number }) =>
        playSpecial(
          ...
          cardId,
          'steal',
          (room, actor) => stealCard(room, actor, cardId, targetPlayerId, cardIndex),
          targetPlayerId
        )
    );
    socket.on(
