...
describe('startGame', () => {
  ...
  it('alternates teams so one team never plays twice in a row (2v2)', () => {
    const r = baseRoom();
    r.players = [
      { id: 'r1', name: 'R1', icon: 1, team: 'red', connected: true },
      { id: 'r2', name: 'R2', icon: 2, team: 'red', connected: true },
      { id: 'b1', name: 'B1', icon: 3, team: 'blue', connected: true },
      { id: 'b2', name: 'B2', icon: 4, team: 'blue', connected: true },
    ];
    const started = startGame(r);
    const teamAt = (pid: string) => started.players.find((p) => p.id === pid)!.team;
    const order = started.turnOrder;
    expect(order).toHaveLength(4);
    // Every adjacent pair — including the wrap from the last seat back to the first —
...
describe('playNumberCard', () => {
  it('places chip on a matching empty cell, refills hand, advances turn', () => {
    ...
    expect(r.hands['p1']).toHaveLength(3);
    expect(r.turnOrder[r.currentTurn]).toBe('p2');
    expect(r.lastMove).toEqual({ index: cellIndex, playerId: 'p1', kind: 'place' });
  });
