...
The repo is **RogerChouSPC/my-games**, and this game lives in the `Number Wars/`
...
5. **IMPORTANT — set the root directory:** because the game is in a subfolder, open
   **Settings → Source → Root Directory** and set it to `Number Wars`, then redeploy.
...
### Notes
- The `Number Wars/railway.json` file tells Railway how to build and run the game once the
